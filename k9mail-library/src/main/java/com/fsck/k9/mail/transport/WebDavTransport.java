...
public class WebDavTransport extends Transport {
    private static final Logger log = LoggerFactory.getLogger(WebDavTransport.class);
    ...
    public WebDavTransport(StoreConfig storeConfig) throws MessagingException {
        ...
        if (K9MailLib.isDebug())
            log.debug(">>> New WebDavTransport creation complete");
    }

    @Override
    public void open() throws MessagingException {
        if (K9MailLib.isDebug())
            log.debug(">>> open called on WebDavTransport ");
